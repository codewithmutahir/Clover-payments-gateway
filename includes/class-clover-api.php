...
class Clover_API
{
	 ...
	 * @param int         $quantity         WooCommerce quantity.
	 *
	 * @return array{success:bool,id?:string,message?:string}
	 */
	protected function add_line_item( $clover_order_id, $name, $price_cents, $apply_tax = false, $quantity = 1 ) {
		$quantity = max( 1, (int) $quantity );

		$this->log(
			'Sequential add_line_item: ' . $name
			. ' qty=' . $quantity
			. ' — ALWAYS AD-HOC (no item.id)'
		);

		$attempts = array(
			array( 'apply_tax' => (bool) $apply_tax ),
		);

		if ( $apply_tax ) {
			$attempts[] = array( 'apply_tax' => false );
		}
	...
	/**
	 * Add a pre-built ad-hoc line item payload (used by sequential sync).
	 *
	 * @param string               $clover_order_id Clover order ID.
	 * @param array<string,mixed>  $li              Sanitized line item body.
	 * @return array{success:bool,id?:string,message?:string}
	 */
	protected function add_line_item_from_payload( $clover_order_id, array $li ) {
		$clean = $this->sanitize_line_items_for_api( array( $li ) );
		$body  = isset( $clean[0] ) ? $clean[0] : array();

		if ( empty( $body['name'] ) ) {
			return array(
				'success' => false,
				'message' => __( 'Line item name is required.', 'clover-gateway' ),
			);
		}

		$this->log(
			'Sequential add_line_item payload: ' . wp_json_encode( $body )
		);
	...
	/**
	 * Log if Clover attached catalog inventory or unitQty after sync (causes 1/1 prints).
	 *
	 * @param string   $clover_order_id Clover order ID.
	 * @param WC_Order $order           WooCommerce order.
	 * @return void
	 */
	protected function audit_clover_line_items_after_sync( $clover_order_id, $order ) {
		$fetch = $this->fetch_order_with_line_items( $clover_order_id );
		if ( empty( $fetch['success'] ) || empty( $fetch['order']['lineItems']['elements'] ) ) {
			return;
		}

		foreach ( $fetch['order']['lineItems']['elements'] as $li ) {
			$name = isset( $li['name'] ) ? (string) $li['name'] : '';
			$issues = array();

			if ( ! empty( $li['item']['id'] ) ) {
				$issues[] = 'catalog-linked id=' . $li['item']['id'];
			}
			if ( isset( $li['unitQty'] ) && (int) $li['unitQty'] > 0 ) {
				$issues[] = 'unitQty=' . (int) $li['unitQty'];
			}

			if ( ! empty( $issues ) ) {
				$this->log(
					'POST-SYNC AUDIT [' . $name . ']: ' . implode( ', ', $issues )
					. ' — this causes 1/1 kitchen display',
					'error'
				);
				$order->add_order_note(
					sprintf(
						/* translators: 1: item name, 2: issue details */
						__( 'Clover line "%1$s" has %2$s — kitchen may show 1/1. Check Clover catalog name match or Group Line Items setting.', 'clover-gateway' ),
						$name,
						implode( ', ', $issues )
					)
				);
	 ...
	 * @param int         $quantity     WooCommerce quantity (unitQty omitted when 1).
	 * @param bool        $apply_tax    Whether to embed default tax metadata.
	 * @return array<string,mixed>
	 */
	protected function build_line_item_request_body( $name, $price_cents, $quantity, $apply_tax = false ) {
		$quantity = max( 1, (int) $quantity );

		// Never send unitQty — kitchen printers render it as a "N/N" quantity prefix.
		$body = array(
			'name'  => $name,
		...
		if ($apply_tax && ! empty($this->default_tax_rate_id)) {
			...
			if ($full_rate) {
				...
				if ($rate_val > 0) {
					$line_total_cents  = (int) $price_cents * $quantity;
	...
	/**
	 * Build a kitchen-safe ad-hoc line item name.
	 *
	 * Clover POS auto-matches line item names to catalog inventory. When "Tuesday Deal"
	 * matches a kitchen-tracked inventory item, POS shows x1/1 (fired/total) instead of x1.
	 * An invisible zero-width character breaks exact matching without changing the ticket.
	 *
	 * @param string $name     WooCommerce product name.
	 * @param int    $quantity Line quantity.
	 * @return string
	 */
	protected function clover_kitchen_line_name( $name, $quantity ) {
		$name = (string) $name;
		...
		// Zero-width non-joiner (U+200C) after first character — invisible on printers.
		if ( function_exists( 'mb_substr' ) ) {
			$name = mb_substr( $name, 0, 1 ) . "\u{200C}" . mb_substr( $name, 1 );
		} else {
			$name = substr( $name, 0, 1 ) . "\u{200C}" . substr( $name, 1 );
		}

		return apply_filters( 'clover_gateway_kitchen_line_name', $name, $quantity );
	}

	/**
	 * Build ad-hoc Clover line items for one WooCommerce product row.
	 *
	 * Kitchen printers render unitQty as a "N/N" prefix (e.g. 1/1, 2/2). The only
	 * reliable fix is one API line item per unit: name + unit price, never unitQty.
	 * Clover's "Group line items" setting (Setup → Order Receipts) merges duplicates
	 * into a single "2 Item Name" row on the ticket.
	 *
	 * @param WC_Order_Item_Product $item             Order line item.
	 * @param int                   $quantity         Quantity.
	 * @param int                   $unit_price_cents Per-unit price in cents.
	 * @param int                   $line_total_cents Full line total in cents.
	 * @return array<int,array<string,mixed>>
	 */
	protected function build_adhoc_product_line_items( $item, $quantity, $unit_price_cents, $line_total_cents ) {
		$quantity = max( 1, (int) $quantity );
		$lines    = array();
		$name     = $this->clover_kitchen_line_name( $item->get_name(), 1 );
		...
		if ( ! empty( $this->default_tax_rate_id ) ) {
			...
			if ( $full_rate && ! empty( $full_rate['rate'] ) && (int) $full_rate['rate'] > 0 ) {
				$line_tax_cents = (int) round( $line_total_cents * (int) $full_rate['rate'] / 1000000000 );
				$tax_rates      = array(
					array(
						'id'        => $this->default_tax_rate_id,
		...
		$unit_tax_cents = $quantity > 0 ? (int) floor( $line_tax_cents / $quantity ) : 0;
		$tax_remainder  = $line_tax_cents - ( $unit_tax_cents * $quantity );

		for ( $u = 0; $u < $quantity; $u++ ) {
			$li = array(
				'name'  => $name,
				'price' => (int) $unit_price_cents,
			);

			if ( ! empty( $tax_rates ) && $line_tax_cents > 0 ) {
				$this_unit_tax   = $unit_tax_cents + ( $u < $tax_remainder ? 1 : 0 );
				$li['taxAmount'] = $this_unit_tax;
				$li['taxRates']  = $tax_rates;
			}
	...
	protected function resolve_clover_item_id_for_line_item( $order, $item ) {
		...
		if ( $this->is_clover_item_id_known_invalid( $clover_item_id ) ) {
			return null;
		}
	...
	/**
	 * Whether a Clover API error indicates a missing/invalid inventory item reference.
	 *
	 * @param array $result Response from request_v3().
	 * @return bool
	 */
	protected function is_clover_invalid_item_reference_error( $result ) {
		...
		if ( ! empty( $result['data'] ) && is_array( $result['data'] ) ) {
			if ( ! empty( $result['data']['message'] ) ) {
				$messages[] = (string) $result['data']['message'];
			}
			if ( ! empty( $result['data']['details'] ) ) {
				$messages[] = (string) $result['data']['details'];
			}
		}
		...
		foreach ( $messages as $message ) {
			...
			if (
				...
				|| false !== strpos( $lower, 'referenced item' )
				|| false !== strpos( $lower, 'cart_is_empty_or_missing' )
			) {
				return true;
			}
		}
	...
	/**
	 * Transient key for a Clover item ID known to be invalid for this merchant.
	 *
	 * @param string $clover_item_id Clover inventory item ID.
	 * @return string
	 */
	protected function invalid_clover_item_transient_key( $clover_item_id ) {
		return 'clover_bad_item_' . md5( $this->merchant_id . '|' . trim( (string) $clover_item_id ) );
	}

	/**
	 * Whether this Clover item ID was previously rejected by the API.
	 *
	 * @param string $clover_item_id Clover inventory item ID.
	 * @return bool
	 */
	protected function is_clover_item_id_known_invalid( $clover_item_id ) {
		if ( '' === trim( (string) $clover_item_id ) ) {
			return false;
		}

		return (bool) get_transient( $this->invalid_clover_item_transient_key( $clover_item_id ) );
	}

	/**
	 * Remember a Clover item ID that does not exist so we skip linking on future orders.
	 *
	 * @param string $clover_item_id Clover inventory item ID.
	 */
	protected function mark_clover_item_id_invalid( $clover_item_id ) {
		$clover_item_id = trim( (string) $clover_item_id );
		if ( '' === $clover_item_id ) {
			return;
		}

		set_transient( $this->invalid_clover_item_transient_key( $clover_item_id ), 1, 7 * DAY_IN_SECONDS );
	}

	/**
	 * Whether any atomic line item payload includes an inventory link.
	 *
	 * @param array<int,array<string,mixed>> $line_items Line item payloads.
	 * @return bool
	 */
	protected function atomic_line_items_have_inventory_links( $line_items ) {
	...
	/**
	 * Strip inventory links from atomic line items so Clover treats them as ad-hoc.
	 *
	 * @param array{elements:array<int,array<string,mixed>>} $line_data Atomic lineItems payload.
	 * @return array{elements:array<int,array<string,mixed>>}
	 */
	protected function convert_atomic_line_items_to_adhoc( $line_data ) {
		if ( empty( $line_data['elements'] ) || ! is_array( $line_data['elements'] ) ) {
			return $line_data;
		}

		$elements = array();

		foreach ( $line_data['elements'] as $li ) {
			if ( ! empty( $li['item']['id'] ) ) {
				$invalid_id = (string) $li['item']['id'];
				$this->mark_clover_item_id_invalid( $invalid_id );
				unset( $li['item'] );
				$this->log(
					'Converting to ad-hoc (invalid inventory id=' . $invalid_id . '): '
					. ( isset( $li['name'] ) ? $li['name'] : '(unnamed)' ),
					'warning'
				);
			}
	...
	/**
	 * Whether an atomic order should be retried without inventory links.
	 *
	 * @param array                            $result     API response.
	 * @param array<int,array<string,mixed>>   $line_items Line item payloads.
	 * @return bool
	 */
	protected function should_fallback_atomic_to_adhoc( $result, $line_items ) {
		if ( ! $this->atomic_line_items_have_inventory_links( $line_items ) ) {
			return false;
		}

		return $this->is_clover_invalid_item_reference_error( $result );
	}
	 ...
	 *
	 * @param WC_Order $order       WooCommerce order.
	 * @param bool     $force_adhoc When true, skip inventory linking (ad-hoc only).
	 * @return array{elements:array<int,array<string,mixed>>}
	 */
	protected function build_atomic_line_items( $order, $force_adhoc = false ) {
		$elements = array();
		...
		foreach ($order->get_items() as $item) {
			...
			$line_total_cents = (int) round( $total * 100 );
			$unit_price_cents = (int) round( ( $total / $quantity ) * 100 );
			$product_lines    = $this->build_adhoc_product_line_items( $item, $quantity, $unit_price_cents, $line_total_cents );

			$this->log(
				'Item [' . $item->get_name() . ']: ' . count( $product_lines ) . ' ad-hoc unit line(s)'
				. ' wc_qty=' . $quantity
				. ' unit_price_cents=' . $unit_price_cents
				. ' (no unitQty — prevents 1/1 print prefix)'
			);

			foreach ( $product_lines as $product_line ) {
				$elements[] = $product_line;
			}
		}
	...
	{
		$data = $this->build_atomic_line_items( $order, true );
	...
	/**
	 * Create Clover order via custom order + line_items API (primary path).
	 *
	 * Avoids atomic_order, which can attach catalog inventory by product name and
	 * cause kitchen/POS to show x1/1 fire-fraction display instead of plain quantity.
	 *
	 * @param WC_Order $order WooCommerce order.
	...
	{
		// Custom order + line_items API only — avoids atomic_order catalog matching.
		$line_data  = $this->build_atomic_line_items( $order, true );
		$line_items = $this->sanitize_line_items_for_api( $line_data['elements'] );
		$line_data  = array( 'elements' => $line_items );
		...
		foreach ( $line_items as $index => $li ) {
			...
			$this->log( 'Sequential line item #' . ( $index + 1 ) . ': ' . $name . ' price_cents=' . $price );

			$added = $this->add_line_item_from_payload( $clover_order_id, $li );
		...
		// Sequential line items take longer to commit than atomic — wait before print.
		sleep( 2 );
	...
	/**
	 * Fallback: create Clover order via atomic_order API (single request).
	 *
	 * @param WC_Order $order WooCommerce order.
	 * @return array|false Result array on success, false on failure.
	 */
	protected function create_atomic_order($order)
	{
		...
		$this->log( 'Atomic fallback payload: ' . wp_json_encode( $line_data ) );
		...
		$parsed = $this->parse_order_create_api_result( $order, $result, $tax_cents, $total_cents, count( $line_items ) );
		if ( is_array( $parsed ) && ! $this->is_order_creation_failure( $parsed ) ) {
			$parsed['used_sequential'] = false;
		}
	...
	public function create_order_with_items($order)
	{
		...
		// Prefer custom order + line_items — atomic_order can attach catalog inventory by name.
		$created           = $this->create_order_sequential($order);
		$sequential_failure = null;

		if ($this->is_order_creation_failure($created)) {
			$sequential_failure = $created;
			$this->log(
				'Sequential order creation failed for WC order #' . $order->get_order_number()
					. ' — falling back to atomic. '
		...
		if ($this->is_order_creation_failure($created)) {
			...
			} elseif ($sequential_failure && ! empty($sequential_failure['message'])) {
				$message = (string) $sequential_failure['message'];
			}
			if ('' === $message) {
		...
		if ( function_exists( 'clover_gateway_is_order_debug_enabled' ) && clover_gateway_is_order_debug_enabled() ) {
			$force_adhoc = ! empty( $created['used_sequential'] );
			$snapshot    = $this->build_atomic_line_items( $order, true );
			$order->update_meta_data( '_clover_debug_sent_payload', $snapshot['elements'] );
			$order->update_meta_data( '_clover_debug_sync_path', $force_adhoc ? 'sequential' : 'atomic' );
			$order->update_meta_data( '_clover_line_format', 'unit-expand-v1.0.7' );
			$order->update_meta_data( '_clover_debug_saved_at', current_time( 'mysql' ) );
			$order->save();
		}

		$this->audit_clover_line_items_after_sync( $created['clover_order_id'], $order );
		...
		return array(
			...
			'used_sequential' => ! empty( $created['used_sequential'] ),
		);
	}
	...
	public function fire_order( $clover_order_id, $wc_order_id = null, $used_sequential = false ) {
		...
		$this->log( sprintf(
			'fire_order() called: clover_order=%s wc_order=%s sequential=%s',
			$clover_order_id,
			$wc_order_id ? $wc_order_id : 'null',
			$used_sequential ? 'yes' : 'no'
		) );
		...
		// Wait for Clover to commit line items before print. Sequential path already slept 2s.
		if ( ! $used_sequential ) {
			usleep( 1500000 ); // 1.5 seconds for atomic orders
		}

		foreach ( $categories as $category ) {
			// Delay between print categories (ORDER, RECEIPT) to avoid race conditions.
			usleep( 800000 ); // 800ms
	...
	/**
	 * Fetch a Clover order with line items expanded (debug panel).
	 *
	 * @param string $clover_order_id Clover order ID.
	 * @return array{success:bool,order?:array,message?:string}
	 */
	public function fetch_order_with_line_items( $clover_order_id ) {
		$result = $this->request_v3(
			'GET',
			'/merchants/' . rawurlencode( $this->merchant_id ) . '/orders/' . rawurlencode( $clover_order_id ),
			array(),
			array( 'expand' => 'lineItems.item' )
		);
	...
	/**
	 * Build normalized sent line items for the debug panel.
	 *
	 * @param WC_Order $order       WooCommerce order.
	 * @param bool     $force_adhoc Skip inventory linking.
	 * @return array<int,array<string,mixed>>
	 */
	public function get_sent_line_items_for_debug( $order, $force_adhoc = false ) {
		$this->ensure_order_totals( $order );
		$data = $this->build_atomic_line_items( $order, $force_adhoc );

		return $this->normalize_line_items_for_debug( $data['elements'] );
	}

	/**
	 * Fetch and normalize Clover line items for the debug panel.
	 *
	 * @param string $clover_order_id Clover order ID.
	 * @return array{success:bool,rows?:array,order_state?:string,message?:string}
	 */
	public function get_clover_line_items_for_debug( $clover_order_id ) {
		$fetch = $this->fetch_order_with_line_items( $clover_order_id );
		...
		$elements = array();
		if ( ! empty( $fetch['order']['lineItems']['elements'] ) && is_array( $fetch['order']['lineItems']['elements'] ) ) {
			$elements = $fetch['order']['lineItems']['elements'];
		}

		return array(
			'success'     => true,
			'rows'        => $this->normalize_line_items_for_debug( $elements ),
			'order_state' => isset( $fetch['order']['state'] ) ? (string) $fetch['order']['state'] : '',
		);
	}

	/**
	 * Normalize line item payloads for debug comparison.
	 *
	 * @param array<int,array<string,mixed>> $elements Line items.
	 * @return array<int,array<string,mixed>>
	 */
	public function normalize_line_items_for_debug( $elements ) {
		$rows = array();
		...
		foreach ( $elements as $li ) {
			...
			if ( ! empty( $li['name'] ) ) {
				$name = (string) $li['name'];
			} elseif ( ! empty( $li['item']['name'] ) ) {
				$name = (string) $li['item']['name'];
			}

			$unit_qty = isset( $li['unitQty'] ) ? (int) $li['unitQty'] : 0;
			$qty_sold = isset( $li['quantitySold'] ) ? (float) $li['quantitySold'] : ( $unit_qty > 0 ? $unit_qty / 1000 : 1 );
			$printed  = array_key_exists( 'printed', $li ) ? (bool) $li['printed'] : null;

			$item_id = '';
			if ( ! empty( $li['item']['id'] ) ) {
				$item_id = (string) $li['item']['id'];
			} elseif ( ! empty( $li['item'] ) && is_string( $li['item'] ) ) {
				$item_id = $li['item'];
			}
			...
			$rows[] = array(
				...
				'display_qty'   => $this->format_pos_quantity_display( $unit_qty, $qty_sold, $printed ),
				'printed'       => $printed,
				'item_id'       => $item_id,
				'type'          => $item_id ? 'linked' : 'ad-hoc',
				'price_cents'   => isset( $li['price'] ) ? (int) $li['price'] : 0,
			);
		}
	...
	/**
	 * Estimate POS quantity label from Clover line item fields.
	 *
	 * @param int        $unit_qty     Clover milli-units.
	 * @param float      $qty_sold     quantitySold.
	 * @param bool|null  $printed      printed flag.
	 * @return string
	 */
	protected function format_pos_quantity_display( $unit_qty, $qty_sold, $printed ) {
		$qty = (int) round( $qty_sold > 0 ? $qty_sold : ( $unit_qty > 0 ? $unit_qty / 1000 : 1 ) );
	...
	/**
	 * Compare sent vs Clover line items for the debug panel.
	 *
	 * @param array<int,array<string,mixed>> $sent_rows   Normalized sent rows.
	 * @param array<int,array<string,mixed>> $clover_rows Normalized Clover rows.
	 * @return array<int,array<string,mixed>>
	 */
	public function build_debug_comparison( $sent_rows, $clover_rows ) {
		$comparison = array();
		$max        = max( count( $sent_rows ), count( $clover_rows ) );

		for ( $i = 0; $i < $max; $i++ ) {
			$sent   = isset( $sent_rows[ $i ] ) ? $sent_rows[ $i ] : null;
			$clover = isset( $clover_rows[ $i ] ) ? $clover_rows[ $i ] : null;
			$status = 'ok';
			$notes  = array();

			if ( ! $sent || ! $clover ) {
				$status  = 'error';
				$notes[] = __( 'Line count mismatch between sent and Clover', 'clover-gateway' );
			} else {
				if ( (int) $sent['unit_qty'] !== (int) $clover['unit_qty'] ) {
					$status  = 'warn';
					$notes[] = __( 'unitQty mismatch', 'clover-gateway' );
				}
				if ( abs( (float) $sent['quantity_sold'] - (float) $clover['quantity_sold'] ) > 0.01 ) {
					$status  = 'warn';
					$notes[] = __( 'quantitySold mismatch', 'clover-gateway' );
				}
				if ( false === $clover['printed'] ) {
					$status  = 'warn';
					$notes[] = __( 'not printed — POS may show x1/0', 'clover-gateway' );
				}
				if ( ! empty( $clover['item_id'] ) ) {
					$status  = 'error';
					$notes[] = __( 'Clover linked catalog inventory — causes x1/1 on POS/kitchen', 'clover-gateway' );
				}
				if ( 'x1/0' === $clover['display_qty'] ) {
					$status  = 'error';
					$notes[] = __( 'POS quantity x1/0', 'clover-gateway' );
				}
			}

			$comparison[] = array(
				'index'  => $i + 1,
				'name'   => $sent ? $sent['name'] : ( $clover ? $clover['name'] : '' ),
				'sent'   => $sent,
				'clover' => $clover,
				'status' => $status,
				'notes'  => implode( '; ', $notes ),
